...
function auth_on_register_common(db_library, reg)
   ...
   if reg.username ~= nil and reg.password ~= nil then
      ...
      -- extract mountpoint from username, pattern: <mountpoint>:<username>.
      sepIndex = string.find(reg.username, ':', 1, true) 

      -- if no separator found, reject authentication
      if sepIndex ~= nil then
         reg.mountpoint = string.sub(reg.username, 1, sepIndex - 1)
         reg.username = string.sub(reg.username, sepIndex + 1)

         if reg.mountpoint == nil or reg.mountpoint == "" then
            return false
         end
      else
         return false
      ...
      if client_side_hashing(method) then
         ...
         if validate_result_client_side(results, reg) then
            -- apply mountpoint and client_id substitutions
            return {
               subscriber_id = {
         ...
         if validate_result_server_side(results, reg) then
            -- apply mountpoint and client_id substitutions
            return {
               subscriber_id = {
                  mountpoint = reg.mountpoint,
